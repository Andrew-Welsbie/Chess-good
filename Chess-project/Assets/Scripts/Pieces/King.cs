...
public class King : Piece
{
    public override List<Vector2Int> MoveLocations(Vector2Int gridPoint)
    {
        ...
        Vector2Int forwardAttc = new Vector2Int(gridPoint.x, gridPoint.y + forwardDirection);
        if (GameManager.instance.PieceAtGrid(forwardAttc))
        {
            locations.Add(forwardAttc);
        }

        Vector2Int forwardRightAttc = new Vector2Int(gridPoint.x + 1, gridPoint.y + forwardDirection);
        if (GameManager.instance.PieceAtGrid(forwardRightAttc))
        {
            locations.Add(forwardRightAttc);
        }

        Vector2Int forwardLeftAttc = new Vector2Int(gridPoint.x - 1, gridPoint.y + forwardDirection);
        if (GameManager.instance.PieceAtGrid(forwardLeftAttc))
        {
            locations.Add(forwardLeftAttc);
        }
        Vector2Int leftAttc = new Vector2Int(gridPoint.x - 1, gridPoint.y);
        if (GameManager.instance.PieceAtGrid(leftAttc))
        {
            locations.Add(leftAttc);
        }
        Vector2Int rightAttc = new Vector2Int(gridPoint.x + 1, gridPoint.y);
